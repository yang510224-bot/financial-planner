...
export default function Step7FuturePlanning({ data }) {
  ...
  const results = useMemo(() => {
    if (params.planType === 'monthly') {
      ...
      return calculateLumpSumPlan(params.lumpSumAmount, params.dividendRate, currentAge);
    } else if (params.planType === 'combined') {
      return calculateCombinedPlan(params.monthlyPremium, params.investmentPeriod, params.annualRate, params.lumpSumAmount, params.dividendRate, currentAge, gender);
    }
    return null;
    ...
    if (params.planType === 'monthly') {
      return (
        <div style={{ marginTop: '24px' }}>
          <h3 style={{ borderBottom: '2px solid var(--primary-color)', paddingBottom: '8px', color: 'var(--primary-dark)' }}>【每月投保規劃】</h3>
          <p>📝 每月投入：{params.monthlyPremium || 0} 元 | 投保期限：{params.investmentPeriod} 年期 | 假設基金年化：{params.annualRate}%</p>
    ...
    if (params.planType === 'lumpSum') {
      return (
        <div style={{ marginTop: '24px' }}>
          ...
          <p>📝 一次投入：{params.lumpSumAmount || 0} 萬元 | 年配息率：{params.dividendRate}%</p>
    ...
    if (params.planType === 'combined') {
      return (
        <div style={{ marginTop: '24px' }}>
          ...
          <p>📝 躉繳投資：{params.lumpSumAmount || 0} 萬元 | 每月投保：{params.monthlyPremium || 0} 元 ({params.investmentPeriod}年) | 年配息率：{params.dividendRate}%</p>
          ...
          <div style={{ background: '#FFF8E1', padding: '16px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #FFC107' }}>
            ...
            <div>月投保支出：<span style={{color: 'var(--danger-color)'}}>-{Math.round(results.immediate.monthlyPremiumExpense).toLocaleString()}元</span></div>
          ...
          <div className="flex gap-4" style={{ flexWrap: 'wrap' }}>
            {selectedYears.map(year => (
              <div key={year} style={{ flex: '1 1 30%', border: '1px solid var(--border-color)', borderRadius: '8px', background: 'var(--white)', padding: '16px' }}>
                ...
                <div>月投保累積: <span style={{color: 'var(--success-color)'}}>+{results[year].assetAccumulation.toFixed(2)}萬</span></div>
                <hr style={{ borderTop: '1px solid #ccc', margin: '8px 0' }} />
                <div style={{fontWeight: 'bold'}}>總資產增加: +{results[year].totalAssetIncrease.toFixed(2)}萬元</div>
      ...
      <div style={{ background: '#faf9f6', padding: '16px', borderRadius: '8px', marginBottom: '32px', border: '1px solid var(--border-color)' }}>
        {(params.planType === 'monthly' || params.planType === 'combined') && (
          <div style={{ marginBottom: '16px' }}>
            <h4 style={{ margin: '0 0 12px 0' }}>【每月投保設定】</h4>
            <div className="flex gap-4" style={{ flexWrap: 'wrap' }}>
              ...
              <div className="form-group" style={{ flex: '1 1 30%' }}>
                <label className="form-label">投保期限 (年)</label>
